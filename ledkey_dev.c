...
#endif
	wake_up_interruptible(&WaitQueue_Read);// 깨움
	return IRQ_HANDLED;	
}
static int gpioLedInit(void)
{
...
//	kbuf = pKeyData->keyNumber;
	...
	if (pKeyData->keyNumber == 0)
	{
		...
		{
			wait_event_interruptible(WaitQueue_Read, pKeyData->keyNumber);	// 잠 듦
			//wait_event_interruptible_timeout(WaitQueue_Read, pKeyData->keyNumber, 100);	// 100 * 1 / HZ = 100 * 0.01 = 1sec 1초마다 깨어나겠음
		}
	}
...
#endif
    if(wait->_key & POLLIN)
        poll_wait(filp, &WaitQueue_Read, wait);	// poll 호출시 waitQueue로 잠듦 wait 시간에 맞춰 다시 깸
    if(pKeyData->keyNumber > 0)
        mask = POLLIN;
